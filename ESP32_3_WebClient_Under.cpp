...
HardwareSerial SerialSensorUnder(2);  // UART2
...
// 感測器數據緩衝區
unsigned char buffer_RTTUnder[4] = {0};
...
void setup() {
  ...
  SerialSensorUnder.begin(BAUD_RATE, SERIAL_8N1, RXD2, TXD2);
...
void loop() {
  ...
  if (readSensorData(SerialSensorUnder, buffer_RTTUnder)) {
    if (validateChecksum(buffer_RTTUnder)) {
      int newDistance = extractDistance(buffer_RTTUnder);
      if (newDistance >= 0 && newDistance <= MAX_DISTANCE_CM) {
        currentDistanceUnder = newDistance;
      } else {
        currentDistanceUnder = -1; // 重置為無效值
      }
    } else {
      currentDistanceUnder = -1; // 重置為無效值
    }
