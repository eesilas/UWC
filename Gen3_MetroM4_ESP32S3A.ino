// 第一块 Metro ESP32-S3 - 2个水下超声传感器
#define TRIGGER_CMD 0x55
const int baudRate = 115200;

HardwareSerial SerialUltra1(1);
HardwareSerial SerialUltra2(2);
...
void setup() {
  ...
  Serial.println("ESP32-S3 #1: 2个水下超声已就绪");
}
...
void loop() {
  ...
  Serial.print("U1: "); Serial.print(ultra1.stable_mm);
  if (ultra1.stable_mm == 0) Serial.print(" mm (no)");
  else Serial.print(" mm");

  Serial.print(" | U2: "); Serial.print(ultra2.stable_mm);
  if (ultra2.stable_mm == 0) Serial.print(" mm (no)");
  else Serial.print(" mm");
